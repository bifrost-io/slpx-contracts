...
contract AstarXcmAction is IXcmAction, OwnableUpgradeable , PausableUpgradeable {
    ...
    address constant public XCM_ADDRESS = 0x0000000000000000000000000000000000005004;
    ...
    function initialize(uint256 _bifrostTransactionFee,uint64 _transactWeight) public initializer {
        super.__Ownable_init();
        xcm = XCM(XCM_ADDRESS);
        setBifrostTransactionFee(_bifrostTransactionFee,_transactWeight);
    }

    function setBifrostTransactionFee(uint256 _bifrostTransactionFee,uint64 _transactWeight) onlyOwner public {
        require(_bifrostTransactionFee <= 1000000000000, "Transaction Fee too large");
    ...
    function setAssetAddressToMinimumValue(address assetAddress,uint256 minimumValue) onlyOwner public {
        assetAddressToMinimumValue[assetAddress] = minimumValue;
    }

    function setAssetAddressToCurrencyId(address assetAddress,bytes2 currencyId) onlyOwner public {
    ...
    function mintVNativeAsset() payable external override whenNotPaused {
        ...
        bytes memory callcode =  BuildCallData.buildMintCallBytes(_msgSender() , ASTR_BYTES,ASTAR_CHAIN);
    ...
    function redeemAsset(address vAssetAddress, uint256 amount) external override whenNotPaused {
        ...
        bytes memory callcode = BuildCallData.buildRedeemCallBytes(_msgSender(),vtoken,ASTAR_CHAIN);
        // xcm transact
        xcm.remote_transact(BIFROST_PARA_ID, false, BNC_ADDRESS, bifrostTransactionFee, callcode, transactWeight);
    ...
    function swapAssetsForExactAssets(address assetInAddress, address assetOutAddress,uint256 assetInAmount, uint128 assetOutMin) external override whenNotPaused {
        ...
        require(assetIn != bytes2(0) &&  assetOut != bytes2(0), "Invalid currencyId");

        xcmTransferAsset(assetInAddress, assetInAmount);

        bytes memory callcode =  BuildCallData.buildSwapCallBytes(_msgSender(), assetIn,assetOut,assetOutMin,ASTAR_CHAIN);
        // xcm transact
        xcm.remote_transact(BIFROST_PARA_ID, false, BNC_ADDRESS, bifrostTransactionFee, callcode, transactWeight);
        emit Swap(_msgSender(), assetInAddress, assetOutAddress, assetInAmount,assetOutMin,callcode);
    }
    ...
    function swapAssetsForExactNativeAssets(address assetInAddress, uint256 assetInAmount, uint128 assetOutMin) external override whenNotPaused {
        ...
        bytes memory callcode =  BuildCallData.buildSwapCallBytes(_msgSender() , assetIn,ASTR_BYTES,assetOutMin,ASTAR_CHAIN);
        // xcm transact
        xcm.remote_transact(BIFROST_PARA_ID, false, BNC_ADDRESS, bifrostTransactionFee, callcode, transactWeight);
        emit Swap(_msgSender(), assetInAddress, NATIVE_ASSET_ADDRESS, assetInAmount,assetOutMin,callcode);
    }
    ...
    function swapNativeAssetsForExactAssets(address assetOutAddress, uint128 assetOutMin) payable external override whenNotPaused {
        ...
        bytes memory callcode = BuildCallData.buildSwapCallBytes(_msgSender() , ASTR_BYTES,assetOut,assetOutMin,ASTAR_CHAIN);
        // xcm transact
        xcm.remote_transact(BIFROST_PARA_ID, false, BNC_ADDRESS, bifrostTransactionFee, callcode, transactWeight);
        emit Swap(_msgSender(), NATIVE_ASSET_ADDRESS, assetOutAddress, msg.value,assetOutMin,callcode);
    }
}