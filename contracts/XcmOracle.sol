...
contract XcmOracle is OwnableUpgradeable, PausableUpgradeable {
    ...
    function initialize(address _SlxAddress, address _SovereignAddress) public initializer {
        __Ownable_init();
        __Pausable_init();
    ...
    function getVTokenByToken(address _assetAddress, uint256 _assetAmount) whenNotPaused public view returns (uint256) {
    ...
    function getTokenByVToken(address _assetAddress, uint256 _vAssetAmount) whenNotPaused public view returns (uint256) {
        bytes2 currencyId = getCurrencyIdByAssetAddress(_assetAddress);
        PoolInfo memory poolInfo = tokenPool[currencyId];
        require(poolInfo.vAssetAmount != 0 && poolInfo.assetAmount != 0, "Not ready");
    ...
    function getCurrencyIdByAssetAddress(address _assetAddress) public view returns (bytes2) {
        (bytes2 currencyId, uint256 _d) = MoonbeamSlpx(slxAddress).addressToAssetInfo(_assetAddress);
