...
contract MoonbeamXcmAction is IXcmAction, OwnableUpgradeable , PausableUpgradeable {

  address constant public NATIVE_ASSET_ADDRESS = 0x0000000000000000000000000000000000000802;
  address constant public XCM_TRANSACTORV2_ADDRESS = 0x000000000000000000000000000000000000080D;
  address constant public XTOKENS = 0x0000000000000000000000000000000000000804;
    bytes1 constant public TARGETCHAIN = 0x01;
  ...
  Xtokens xtokens;
    ...
    function initialize(address _BNCAddress,uint32 _bifrostParaId, bytes2 _nativeCurrencyId) public initializer {
        super.__Ownable_init();
        require(_BNCAddress != address(0), "Invalid address");
        require(_bifrostParaId == 2001 || _bifrostParaId == 2030, "Invalid bifrostParaId");
        require(_nativeCurrencyId == 0x020a || _nativeCurrencyId == 0x0801, "Invalid nativeCurrencyId");
   ...
   function setFee(uint64 _xtokenWeight,uint64 _transactRequiredWeightAtMost,uint64 _overallWeight,uint256 _feeAmount) onlyOwner public {
        require(_xtokenWeight <= 10000000000, "xtokenWeight too large");
        require(_transactRequiredWeightAtMost <= 10000000000, "transactRequiredWeightAtMost too large");
        require(_overallWeight <= 10000000000, "transactRequiredWeightAtMost too large");
    ...
    function setAssetAddressToMinimumValue(address assetAddress,uint256 minimumValue) onlyOwner external {
    ...
    function xcmTransferAsset(address assetAddress, uint256 amount) internal  {
        require(assetAddress != address(0), "Invalid assetAddress");
        require(assetAddressToMinimumValue[assetAddress] <= amount, "Less than MinimumValue");
        bytes32 publicKey = AddressToAccount.AddressToSubstrateAccount(_msgSender());
  ...
  function xcmTransferNativeAsset(uint256 amount) internal {
    require(assetAddressToMinimumValue[NATIVE_ASSET_ADDRESS] <= amount, "Less than MinimumValue");
    bytes32 publicKey = AddressToAccount.AddressToSubstrateAccount(_msgSender());
  ...
  function mintVNativeAsset() payable external override whenNotPaused {
    ...
    bytes memory callData = BuildCallData.buildMintCallBytes(_msgSender(), nativeCurrencyId,TARGETCHAIN);
    // XCM Transact
    ...
    function mintVAsset(address assetAddress,uint256 amount) external  override whenNotPaused {
        ...
        XcmTransactorV2.Multilocation memory dest = getXcmTransactorDestination();
        // Build bifrost xcm-action mint call data
        bytes memory callData = BuildCallData.buildMintCallBytes(_msgSender(), token,TARGETCHAIN);
        // XCM Transact
        xcmtransactor.transactThroughSigned(
            dest,
            BNCAddress,
  ...
  function redeemAsset(address vAssetAddress, uint256 amount) external  override whenNotPaused {
    ...
    bytes memory callData = BuildCallData.buildRedeemCallBytes(_msgSender(), vtoken,TARGETCHAIN);
    xcmtransactor.transactThroughSigned(
        dest,
        BNCAddress,
        transactRequiredWeightAtMost,
  ...
  function swapAssetsForExactAssets(address assetInAddress, address assetOutAddress,uint256 assetInAmount, uint128 assetOutMin) external override whenNotPaused {
       ...
       XcmTransactorV2.Multilocation memory dest = getXcmTransactorDestination();
        ...
        bytes memory callData =  BuildCallData.buildSwapCallBytes(_msgSender() , assetIn,assetOut,assetOutMin,TARGETCHAIN);
        xcmtransactor.transactThroughSigned(
            dest,
            BNCAddress,
            transactRequiredWeightAtMost,
        ...
        emit Swap(_msgSender(), assetInAddress, assetOutAddress, assetInAmount,assetOutMin,callData);
    }
    ...
    function swapAssetsForExactNativeAssets(address assetInAddress, uint256 assetInAmount, uint128 assetOutMin) external override whenNotPaused {
        ...
        bytes memory callData =  BuildCallData.buildSwapCallBytes(_msgSender() , assetIn,nativeCurrencyId,assetOutMin,TARGETCHAIN);
        xcmtransactor.transactThroughSigned(
            dest,
            BNCAddress,
            transactRequiredWeightAtMost,
        ...
        emit Swap(_msgSender(), assetInAddress, NATIVE_ASSET_ADDRESS, assetInAmount,assetOutMin,callData);
    }
    ...
    function swapNativeAssetsForExactAssets(address assetOutAddress, uint128 assetOutMin) payable external override whenNotPaused {
       ...
       XcmTransactorV2.Multilocation memory dest = getXcmTransactorDestination();
        ...
        bytes memory callData =  BuildCallData.buildSwapCallBytes(_msgSender() , nativeCurrencyId,assetOut,assetOutMin,TARGETCHAIN);
        xcmtransactor.transactThroughSigned(
            dest,
            BNCAddress,
            transactRequiredWeightAtMost,
        ...
        emit Swap(_msgSender(), NATIVE_ASSET_ADDRESS, assetOutAddress, msg.value,assetOutMin, callData);
    }
