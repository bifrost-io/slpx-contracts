...
contract MoonbeamXcmAction is IXcmAction, Ownable {
  ...
  function xcmTransferNativeAsset(uint256 amount) internal {
    ...
    Xtokens.Multilocation memory dest_account = getXtokensDestination(publicKey);
    xtokens.transfer(Constants.GLMR_ADDRESS, amount, dest_account, xtokenWeight);
  }
  ...
  function mintVNativeAsset() payable external override {
    ...
    bytes memory callData = BuildCallData.buildMintCallBytes(msg.sender, Constants.GLMR_BYTES,Constants.MOOBEAM_CHAIN);
    // XCM Transact
    xcmtransactor.transactThroughSigned(
        dest,
        Constants.MOONBEAM_BNC_ADDRESS,
        transactRequiredWeightAtMost,
        callData,
        feeAmount,
        overallWeight
    );
    emit Mint(msg.sender, Constants.GLMR_ADDRESS, msg.value, callData);
  }
  ...
  function swapAssetsForExactAssets(address assetInAddress, address assetOutAddress,uint256 assetInAmount, uint128 assetOutMin) external override {
    ...
    function swapAssetsForExactNativeAssets(address assetInAddress, uint256 assetInAmount, uint128 assetOutMin) external override {
        ...
        bytes memory callData =  BuildCallData.buildSwapCallBytes(msg.sender , assetIn,Constants.GLMR_BYTES,assetOutMin,Constants.MOOBEAM_CHAIN);
        xcmtransactor.transactThroughSigned(
            dest,
            Constants.MOONBEAM_BNC_ADDRESS,
            transactRequiredWeightAtMost,
        ...
        emit Swap(msg.sender, assetInAddress, Constants.GLMR_ADDRESS, assetInAmount,assetOutMin,callData);
    }
    ...
    function swapNativeAssetsForExactAssets(address assetOutAddress, uint128 assetOutMin) payable external override {
       ...
       XcmTransactorV2.Multilocation memory dest = getXcmTransactorDestination();
        ...
        bytes memory callData =  BuildCallData.buildSwapCallBytes(msg.sender , Constants.GLMR_BYTES,assetOut,assetOutMin,Constants.MOOBEAM_CHAIN);
        xcmtransactor.transactThroughSigned(
            dest,
            Constants.MOONBEAM_BNC_ADDRESS,
            transactRequiredWeightAtMost,
        ...
        emit Swap(msg.sender, Constants.GLMR_ADDRESS, assetOutAddress, msg.value,assetOutMin, callData);
    }

    function getXtokensDestination(bytes32 publicKey) internal pure returns (Xtokens.Multilocation memory) {
      bytes[] memory interior = new bytes[](2);
      // Parachain: 2001/2030
        interior[0] = bytes.concat(hex"00", bytes4(Constants.BIFROST_POLKADOT_PARA_ID));
