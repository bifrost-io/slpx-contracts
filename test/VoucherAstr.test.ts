...
describe("VoucherAstrOFT: ", function () {
  const localChainId = 1;
  const remoteChainId = 2;
  const sharedDecimals = 6;

  let LZEndpointMock, ERC20, VoucherAstrProxyOFT, VoucherAstrOFT;
  let localEndpoint,
    remoteEndpoint,
    localOFT,
    remoteOFT,
  ...
  before(async function () {
    ...
    VoucherAstrProxyOFT = await ethers.getContractFactory(
      "VoucherAstrProxyOFT"
    );
    VoucherAstrOFT = await ethers.getContractFactory("VoucherAstrOFT");
  ...
  beforeEach(async function () {
    ...
    localOFT = await VoucherAstrProxyOFT.deploy(
      erc20.address,
      localEndpoint.address
    );
    console.log("localOFT", localOFT.address);
    remoteOFT = await VoucherAstrOFT.deploy(remoteEndpoint.address);
    console.log("remoteOFT", remoteOFT.address);
    ...
    remotePath = ethers.utils.solidityPack(
      ["address", "address"],
      [remoteOFT.address, localOFT.address]
    );
    localPath = ethers.utils.solidityPack(
      ["address", "address"],
      [localOFT.address, remoteOFT.address]
    );
  ...
  it("send tokens from proxy oft and receive them back", async function () {
    ...
    nativeFee = (
      await remoteOFT.estimateSendFee(
        localChainId,
        aliceAddressBytes32,
        halfAmount,
        false,
  ...
  it("total outbound amount overflow", async function () {
    ...
    await erc20
      .connect(alice)
      .approve(localOFT.address, ethers.constants.MaxUint256);
    const bobAddressBytes32 = ethers.utils.defaultAbiCoder.encode(
      ["address"],
      [bob.address]
    );
