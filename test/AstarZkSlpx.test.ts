...
describe("NativeOFTWithFee: ", function () {
    ...
    beforeEach(async function () {
        ...
        nativeOFTWithFee = await NativeOFTWithFee.deploy("Shibuya Token", "SBY", 6, localEndpoint.address)
        remoteOFTWithFee = await OFTWithFee.deploy("LayerZero Shibuya Token", "LSBY", 6, remoteEndpoint.address)
        ...
        receiverBytes32 = ethers.utils.defaultAbiCoder.encode(["address"], [receiver.address])

        slpx.initialize(remoteOFTWithFee.address, remoteOFT.address, receiverBytes32)
        receiver.initialize(nativeOFTWithFee.address, localOFT.address, slpx.address)
    ...
    it("mint", async function () {
        ...
        // estimate nativeFees
        const payload = ethers.utils.solidityPack(["address", "uint8"], [owner.address, 0])
        nativeFee = (await nativeOFTWithFee.estimateSendAndCallFee(localChainId, receiverBytes32, totalAmount, payload, 700000, false, ethers.utils.solidityPack(["uint16", "uint256"], [1, 1200000])))
            .nativeFee
    ...
    it("redeem()", async function () {
        ...
        const bobAddressBytes32 = ethers.utils.defaultAbiCoder.encode(["address"], [owner.address])
        ...
        // estimate nativeFees
        const payload = ethers.utils.solidityPack(["address", "uint8"], [owner.address, 1])
        nativeFee = (await nativeOFTWithFee.estimateSendAndCallFee(localChainId, receiverBytes32, amount, payload, 700000, false, ethers.utils.solidityPack(["uint16", "uint256"], [1, 1200000])))
            .nativeFee
        console.log("nativeFee: ",nativeFee)
    ...
    it("claimVAstr", async function () {
        ...
        await receiver.claimVAstr(owner.address, defaultAdapterParams,{ value: nativeFee})
        console.log(await remoteOFT.balanceOf(owner.address));
        ...
        console.log(await ethers.provider.getBalance(await receiver.derivativeAddress(owner.address)))
        console.log(await remoteOFTWithFee.balanceOf(owner.address));
        await receiver.claimAstr(owner.address, ethers.utils.parseEther("1"), defaultAdapterParams,{ value: nativeFee })
        console.log(await ethers.provider.getBalance(await receiver.derivativeAddress(owner.address)))
        console.log(await remoteOFTWithFee.balanceOf(owner.address));
    })
})
